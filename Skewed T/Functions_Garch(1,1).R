...
M_fit <- function(x_ini) {  
  ...
  for (f_i in 1:n){
    sig2[1+f_i]=abs(w+a*e2[f_i]+b*sig2[f_i]) # see if I need abs here
  }
  ...
  #E3=(e2_2-sig2_2)/(w+a*e2_1+b*sig2_1)^2
...
IVLS <- function(x_ini) {
  ...
  #sig2[1]=e2[1]=abs(w)
  e1[1]=e2[1]^0.5
  #sig2[1]=e2[1]=theta[3]/(1-theta[4]-theta[5])
  #e1[2:(n+1)]=Y
  e2[2:(n+1)]=Y^2
  for (f_i in 1:n){
    sig2[1+f_i]=abs(w+a*e2[f_i]+b*sig2[f_i])
  }
  #eet=X1[2:n]/(w+a*X)^0.5
  #e1_2=e2[1:n]
  e2_1=e2[1:n]
  e2_2=e2[2:(n+1)]
  sig2_1=sig2[1:n]
...
SP_HR_ss<- function(x_ini) {
 ...
 M_pF=c(0,2)
  ...
  # l_2=W_2*psi_2
  # 
  # P_1=(psi_1-F_t_1%*%t(M_pF%*%solve(M_FF_1)))%*%colMeans(W_1)
  # P_2=(psi_2-F_t_2%*%t(M_pF%*%solve(M_FF_2)))%*%colMeans(W_2)
  # 
  # l_star_1=l_1-P_1
  # l_star_2=l_2-P_2
